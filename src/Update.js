...
import { 
    ...
    updateAddress2,
    updateProfileImage,
    updateCreatedAt } from "./redux/Action";
...
function UpDate() {
    ...
    const [createdAt, setCreatedAt] = useState(all.created_at);
    ...
    const handleCreatedAtChange = (e) => setCreatedAt(e.target.value);
    ...
    return (
        <div className="outBox">
            <div>
                ...
                <button onClick={()=>{dispatch(updateName(name))
                console.log(all.name);
                }}>저장하기</button>
            </div>
            <div>
                <span>프로필 이미지</span>
                <div className="inBox"><img src={`http://xraidev.store:3003${profileImage}`} alt="프로필 이미지"></img></div>
            ...
            <div>
                ...
                onChange={handleCreatedAtChange}></input>
                <button onClick={()=>dispatch(updateCreatedAt(createdAt))}>저장하기</button>
            </div>
